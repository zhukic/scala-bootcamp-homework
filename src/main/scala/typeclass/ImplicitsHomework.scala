...
object ImplicitsHomework extends App {
  ...
  object SuperVipCollections4s {
    ...
    final class MutableBoundedCache[K: GetSizeScore, V: GetSizeScore](maxSizeScore: SizeScore) {
      ...
      def put(key: K, value: V): Unit = {
        val keysSizeScore = map.keys.foldLeft(0) { (acc, item) => acc + item.sizeScore }
        val valuesSizeScore = map.values.foldLeft(0) { (acc, item) => acc + item.sizeScore }

        var currentSizeScore = keysSizeScore + valuesSizeScore

        while (currentSizeScore + key.sizeScore + value.sizeScore > maxSizeScore && map.nonEmpty) {
          val head = map.head
          val headSizeScore = head._1.sizeScore + head._2.sizeScore

          map.remove(head._1)
          currentSizeScore -= headSizeScore
        }
    ...
    object instances {
      ...
      implicit val stringSizeScore: GetSizeScore[String] = s => 12 + s.toCharArray.map(_.sizeScore).sum

      implicit def iterateSizeScore[T: GetSizeScore, U[_]: Iterate]: GetSizeScore[U[T]] = (value: U[T]) => {
        12 + implicitly[Iterate[U]].iterator(value)
          .foldLeft(0) { (acc, item) => acc + item.sizeScore }
      }

      implicit def iterate2SizeScore[T: GetSizeScore, U: GetSizeScore, V[_,_]: Iterate2]: GetSizeScore[V[T, U]] = (value: V[T, U]) => {
        val iterator1SizeScore = implicitly[Iterate2[V]].iterator1(value)
          .foldLeft(0) { (acc, item) => acc + item.sizeScore }
        val iterator2SizeScore = implicitly[Iterate2[V]].iterator2(value)
          .foldLeft(0) { (acc, item) => acc + item.sizeScore }

        12 + iterator1SizeScore + iterator2SizeScore
      }
    }
  }
  ...
  object MyTwitter {
    ...
    implicit val noteSizeScore: GetSizeScore[FbiNote] = fbiNote => {
